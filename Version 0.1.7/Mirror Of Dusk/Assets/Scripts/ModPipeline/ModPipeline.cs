...
#if UNITY_EDITOR
using UnityEditor;
using UnityEditor.Experimental.Rendering.ModPipeline;
#endif
using UnityEngine;
...
namespace UnityEngine.Experimental.Rendering.ModPipeline
{
    ...
    {
        void ExecuteBeforeCameraRender(ModPipeline pipelineInstance, ScriptableRenderContext context, Camera camera);
    }

    public sealed partial class ModPipeline : RenderPipeline
    {
        //CullResults cull;
        ...
        internal struct PipelineSettings
        {
            ...
            public static PipelineSettings Create(ModPipelineAsset asset)
            {
        ...
        public ModPipeline(ModPipelineAsset asset)
        {
            ...
            Shader.globalRenderPipeline = "ModPipeline";
        ...
        public override void Render(ScriptableRenderContext renderContext, Camera[] cameras)
        {
            ...
            {
                BeginCameraRendering(cameras[i]);

                foreach (var beforeCamera in cameras[i].GetComponents<IBeforeCameraRender>())
                    beforeCamera.ExecuteBeforeCameraRender(this, renderContext, cameras[i]);
                RenderSingleCamera(this, renderContext, cameras[i], ref m_CullResults, cameras[i].GetComponent<IRendererSetup>());
            }
        }

        private void RenderSingleCamera(ModPipeline pipelineInstance, ScriptableRenderContext context, Camera camera, ref CullResults cullResults, IRendererSetup setup = null)
        {
            ...
            using (new ProfilingSample(cmd, "Render Camera"))
            {
                ...
                InitializeRenderingData(settings, ref cameraData, ref cullResults, renderer.maxVisibleAdditionalLights, renderer.maxPerObjectAdditionalLights, out renderingData);
                //InitializeRenderingData(settings, ref cameraData, ref cullResults, out renderingData);
        ...
        static void InitializeCameraData(PipelineSettings settings, Camera camera, out CameraData cameraData)
        {
            ...
            ModPipelineAdditionalCameraData additionalCameraData = camera.gameObject.GetComponent<ModPipelineAdditionalCameraData>();
        ...
        /*static void InitializeRenderingData(PipelineSettings settings, ref CameraData cameraData, ref CullResults cullResults, out RenderingData renderingData)
        {
        ...
        static void InitializeShadowData(PipelineSettings settings, List<VisibleLight> visibleLights, bool mainLightCastShadows, bool additionalLightsCastShadows, out ShadowData shadowData)
        {
            ...
            {
                Light light = visibleLights[i].light;
                ModPipelineAdditionalLightData data =
                    (light != null) ? light.gameObject.GetComponent<ModPipelineAdditionalLightData>() : null;
