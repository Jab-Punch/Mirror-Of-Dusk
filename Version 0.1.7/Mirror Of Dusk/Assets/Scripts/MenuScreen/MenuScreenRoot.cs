...
[System.Serializable]
public class MenuDataCollect : UnityEvent<string, int>
{

}

public class MenuScreenRoot : MonoBehaviour, IEventSystemHandler
{
    ...
    public class MenuItemCodes
    {
        ...
        public MenuDataCollect m_MenuDataCollect = new MenuDataCollect();
        [Header("Slot Is Int")]
    ...
    List<GameObject> screenHighlighter;
    ...
    public void initializeCharacterCodes()
    {
    ...
    public void reinitializeColors()
    {
        try
        {
            fighterData = GameObject.Find("FighterData_" + characterName[csPlayerData.characterCode]).GetComponent<FighterData>();
        }
        ...
        if (fighterData != null)
        {
            coloredSquaresA = new Material[fighterData.fighterColorData.Length];
            coloredSquaresB = new Material[fighterData.fighterColorData.Length];
            coloredSquaresC = new Material[fighterData.fighterColorData.Length];
            for (int i = 0; i < coloredSquaresA.Length; i++)
            {
                searchSquare = gameObject.transform.Find("ColorSelectTable/Color" + (i + 1).ToString() + "/ColorSquareA");
                coloredSquaresA[i] = searchSquare.GetComponent<Renderer>().material;
                coloredSquaresA[i].SetColor("_Color_2", fighterData.fighterColorData[i].selectColors.colorA);
                searchSquare = gameObject.transform.Find("ColorSelectTable/Color" + (i + 1).ToString() + "/ColorSquareB");
                coloredSquaresB[i] = searchSquare.GetComponent<Renderer>().material;
                coloredSquaresB[i].SetColor("_Color_2", fighterData.fighterColorData[i].selectColors.colorB);
                searchSquare = gameObject.transform.Find("ColorSelectTable/Color" + (i + 1).ToString() + "/ColorSquareC");
                coloredSquaresC[i] = searchSquare.GetComponent<Renderer>().material;
                coloredSquaresC[i].SetColor("_Color_2", fighterData.fighterColorData[i].selectColors.colorC);
            }
        }
    }
    ...
    private void checkForVerticalScroll()
    {
        if (prevMenuVCode != currentMenuVCode)
        {
            ...
            if (currentVSlot >= slotLimit && !overRun)
            {
                ...
                if (currentMenuVCode >= menuItemCodes.Length - 1) { gameObject.transform.Find("MenuScreenMoreDown").GetComponent<SpriteRenderer>().enabled = false; }
            }
            else if (currentVSlot < 0 && !overRun)
            {
                ...
                gameObject.transform.Find("MenuScreenMoreDown").GetComponent<SpriteRenderer>().enabled = true;
                if (currentMenuVCode <= 0) { gameObject.transform.Find("MenuScreenMoreUp").GetComponent<SpriteRenderer>().enabled = false; }
            }
            else if (currentVSlot <= 0 && overRun)
            {
                currentVSlot = slotLimit - 1;
    ...
    public IEnumerator destroyMenu()
    {
        ...
        yield return new WaitForSeconds(0.4f);
        csPlayerInput.enablePlayerInput = true;
        csPlayerInput.setPlayerMenuMode("");
        Destroy(gameObject);
        yield return null;
    }
    ...
    public void updatedData_Arrow (int setting)
    {
        ...
        } else if (setting == 10)
        {
            ...
            Debug.Log("Barrier:"+gameData.barrierRatio);
        }
        /*if (menuItemCodes[currentMenuVCode].SlotType == ShowValueEnum.Single || menuItemCodes[currentMenuVCode].SlotType == ShowValueEnum.RangeInt || menuItemCodes[currentMenuVCode].SlotType == ShowValueEnum.RangeFloat)
        {
            EventSystem.current.SetSelectedGameObject(menuSlots[currentMenuVCode]);
    ...
    {
        csPlayerInput = GameObject.Find("Player " + pMenuCode).GetComponent<CSPlayerInput>();
        //AudioSource.PlayClipAtPoint(sfxPlayer.cancelNoise, Camera.main.transform.position, 1.0f);
        csPlayerInput.playerInputMode = "CharacterSelect";
        csPlayerInput.enablePlayerInput = false;
        //csPlayerInput.resetScrollFrames();
